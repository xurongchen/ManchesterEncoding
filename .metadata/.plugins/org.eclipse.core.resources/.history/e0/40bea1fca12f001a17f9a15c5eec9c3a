...
//	private ArrayList<String> TapeNames;
	...
	public MultiTapeTuringMachine(ArrayList<TMState> states, ArrayList<Character> alphabet, Character blankSymbol, 
			Transations delta, TMState initialState, ArrayList<TMState> finalStates) throws RuntimeException{
		...
		TransationFrom fisrtFrom = delta.delta.keySet().iterator().next();
		for(Map.Entry<TMTape, Character> tape_character_pair : fisrtFrom.tapeCharacters) {
			this.Tapes.add(tape_character_pair.getKey());
		}
	}
	
	public static class TransationFrom{
		TMState state;
		ArrayList<Map.Entry<TMTape, Character>> tapeCharacters;
		public TransationFrom(TMState state, ArrayList<Map.Entry<TMTape, Character>> tapeCharacters){
			this.state = state;
	...
	public static class TransationTo{
		TMState state;
		ArrayList<Map.Entry<TMTape, Map.Entry<Character, TMTape.ShiftDirection>>> operations;
		public TransationTo(TMState state, ArrayList<Map.Entry<TMTape, Map.Entry<Character, TMTape.ShiftDirection>>> operations) {
			this.state = state;
			this.operations = operations;
		}
	}
	public static class Transations{
		HashMap<TransationFrom, TransationTo> delta;
		public Transations(ArrayList<Map.Entry<TransationFrom, TransationTo>> deltaInArray){
			delta = new HashMap<>();
	...
	public boolean Move() {
		...
		this.nowState = to.state;
		for(Map.Entry<TMTape,Map.Entry<Character,TMTape.ShiftDirection>> tapeOp : to.operations)
			tapeOp.getKey().shift(tapeOp.getValue().getValue(), tapeOp.getValue().getKey());
