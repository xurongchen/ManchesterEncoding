...
public class ManchesterEncoding {
	...
	public TMTape getInput() {
		return input;
	}
	public TMTape getOutput() {
		return output;
	}
	...
	public ManchesterEncoding(String input, boolean IEEE){
		char One = IEEE? '0' :'1';
		char Zero =  IEEE? '1' :'0';
...
//		TMState q1a3 = new TMState("Q1@3");
		...
		for(Character ic : alphabet) {
			for(Character oc : alphabet) {
				deltaInArray.add(new AbstractMap.SimpleEntry<>(
						new TransationFrom(q0a1, FromInfo(ic,oc)), 
						new TransationTo(q0a2, ToInfo(ic, ShiftDirection.No, Zero,ShiftDirection.Right))));
			}
		}
		...
		for(Character ic : alphabet) {
			for(Character oc : alphabet) {
				deltaInArray.add(new AbstractMap.SimpleEntry<>(
						new TransationFrom(q0a2, FromInfo(ic,oc)), 
						new TransationTo(qs, ToInfo(ic, ShiftDirection.No, One,ShiftDirection.Right))));
			}
		}
		...
		for(Character ic : alphabet) {
			for(Character oc : alphabet) {
				deltaInArray.add(new AbstractMap.SimpleEntry<>(
						new TransationFrom(q1a1, FromInfo(ic,oc)), 
						new TransationTo(q1a2, ToInfo(ic, ShiftDirection.No, One,ShiftDirection.Right))));
			}
		}
		...
		for(Character ic : alphabet) {
			for(Character oc : alphabet) {
				deltaInArray.add(new AbstractMap.SimpleEntry<>(
						new TransationFrom(q1a2, FromInfo(ic,oc)), 
						new TransationTo(qs, ToInfo(ic, ShiftDirection.No, Zero,ShiftDirection.Right))));
			}
		}
		
		Transations trans = new Transations(deltaInArray);
		this.turingMachine = new MultiTapeTuringMachine(states, alphabet, 'B', trans, qs, finalStates);
	}
	
	public boolean ExecuteOneStep() {
		return turingMachine.Move();
	}
	
	public void Execute() {
		while(!turingMachine.Move());
	}
