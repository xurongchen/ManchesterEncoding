...
//	JLabel space = new JLabel();
	...
	public MainJFrame() {
		...
		add(panel);
		tapeInput1.setForeground(Color.RED);
		tapeOutput1.setForeground(Color.RED);
//		
...
//		setBackground(Color.WHITE);
	...
	public void CalcDynamicInfo() {
		...
		String input = me.getInput().getTape();
		String output = me.getOutput().getTape();
		int midInput = me.getInput().getPosition();
		int midOutput = me.getOutput().getPosition();

		tapeInput0.setText(midInput<input.length()?input.substring(0, midInput):"");
		tapeInput1.setText(midInput+1<input.length()?input.substring(midInput, midInput+1):"");
		tapeInput2.setText(midInput+1<input.length()?input.substring(midInput+1, input.length()):"");
		
		tapeOutput0.setText(midOutput<output.length()?input.substring(0, midOutput):"");
		tapeOutput1.setText(midOutput+1<output.length()?input.substring(midOutput, midOutput+1):"");
		tapeOutput2.setText(midOutput+1<output.length()?input.substring(midOutput+1, output.length()):"");
